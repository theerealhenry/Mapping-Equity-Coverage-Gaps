...
STAGE6_APPENDED_DOMAIN = build_mod.STAGE6_APPENDED_DOMAIN
...
def test_schema_catalog_has_exactly_233_stage3_rows_with_no_duplicate_column_names(schema_catalog):
    """The 233-row invariant is Stage 3's own scope: the 232 real national-strata-tract-table
    columns plus the one region-only addition (frac_inside_aoi). Stage 6 Step 8 (2026-09-16)
    deliberately appended additional project-computed feature-table columns to this same file,
    tagged `domain="coverage_gap"`, per the Stage 6 guideline's own instruction to keep "one
    continuous, consistent catalog" spanning raw challenge data through derived features — that is
    intended growth, not drift, so this test scopes the invariant to Stage 3's own rows rather than
    breaking every time Stage 6 adds another feature column. The no-duplicate-names check still
    runs file-wide, since a name collision anywhere in the file is a real bug regardless of which
    stage introduced either row."""
    stage3_rows = schema_catalog.loc[schema_catalog["domain"] != STAGE6_APPENDED_DOMAIN]
    assert len(stage3_rows) == 233
...
def test_every_national_schema_catalog_column_has_full_traceability_to_every_upstream_step(
    ...
    Excludes Stage 6's appended project-computed feature columns (`domain="coverage_gap"`): those
    are classified directly in schema_catalog.csv itself, never via these three Stage 3 per-column
    generator CSVs, by design — Stage 6 didn't exist when Steps 2/5/6 ran, so a Stage 6 column
    genuinely has no row in any of them, and that is correct, not a gap to fix."""
    upstream = pd.read_csv(getattr(build_mod, upstream_path_attr))
    national_rows = schema_catalog.loc[
        (~schema_catalog["region_only"]) & (schema_catalog["domain"] != STAGE6_APPENDED_DOMAIN)
    ]
...
def test_committed_schema_catalog_is_reproducible_from_its_own_real_committed_inputs(schema_catalog, tmp_path):
    ...
    # `build_catalog()` only ever knows about Stage 3's own inputs — it has no way to reproduce
    # Stage 6's appended project-computed feature columns (`domain="coverage_gap"`), which are
    # written directly into schema_catalog.csv by hand, not generated by this script. Compare the
    # rebuild against the committed file's Stage 3 subset only; Stage 6's rows are a deliberate,
    # separately-authored addition, not something this generator is meant to reproduce.
    committed_stage3 = schema_catalog.loc[schema_catalog["domain"] != STAGE6_APPENDED_DOMAIN]
    committed_sorted = committed_stage3.sort_values("column_name").reset_index(drop=True)
...
def test_committed_dictionary_markdown_matches_the_committed_schema_catalog(schema_catalog_keep_na, dictionary_text):
    ...
    Scoped to Stage 3's own generator output: `render_data_dictionary_markdown` never produces
    Stage 6's "## Stage 6, Step 8" section (that section, and the project-computed feature rows
    behind it, are hand-authored, not generated by this script — see `STAGE6_APPENDED_DOMAIN`'s
    module-level docstring in `scripts/build_data_dictionary.py`), so this test compares the
    rendered output against only the prefix of the real file that precedes that section, not the
    file's full text. This still catches the exact drift the test exists to catch — Stage 3's
    generic per-domain tables silently diverging from schema_catalog.csv — without demanding this
    generator reproduce hand-written prose it was never meant to own."""
    rebuilt_markdown = build_mod.render_data_dictionary_markdown(schema_catalog_keep_na)
    stage3_dictionary_text = dictionary_text.split("\n## Stage 6")[0]
    assert rebuilt_markdown == stage3_dictionary_text, (
        "DATA_DICTIONARY.md's Stage 3 portion is not reproducible from the committed "
        "schema_catalog.csv via render_data_dictionary_markdown — the two committed files have "
        "drifted apart from each other (a hand-edit to one but not the other, or a rendering-logic "
        "change not reflected in the committed output, are the most likely causes). Regenerate "
        "both with `python -m scripts.build_data_dictionary`, then re-apply Stage 6's hand-authored "
        "addendum section if it was lost."
    )
...
def test_dictionary_domain_table_row_counts_sum_to_232(dictionary_text):
    """Scoped to the Stage 3 portion of the document (before the hand-authored "## Stage 6"
    addendum section). Without this scoping, a Stage 6 subsection heading that happens to be a
    single word (e.g. "### Transport (4 columns)") matches the exact same regex this test uses for
    Stage 3's national-schema domain tables and silently inflates the sum — a real, if narrow,
    heading-format collision between two unrelated sections of the same file, found the first time
    Stage 6 added its own addendum. Scoping to the pre-Stage-6 prefix fixes it at the root rather
    than trying to make Stage 6's headings avoid the pattern by convention."""
    stage3_text = dictionary_text.split("\n## Stage 6")[0]
    counts = [int(n) for n in re.findall(r"^### \S+ \((\d+) columns\)$", stage3_text, flags=re.MULTILINE)]
